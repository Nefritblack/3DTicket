...
Процедура loopTimer() Экспорт 
	...
	Дней = Цел(Факт / 86400); ТаймерДнейСчетчик = ?(Дней, Строка(Дней) + "д. ", "");
...
Функция ОстановитьТаймер()
	...
	Иначе
		...
		Элементы.Счетчик.Видимость = Ложь;
...
Процедура ЗапуститьТаймер()
	...
	Элементы.Счетчик.Видимость = Истина;
...
Процедура пересчетСуммы()
	...
	Сумма = СтоимостьЧаса * ?(План, План, Факт / 3600);
